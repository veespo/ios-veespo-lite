...
- (void)setup
{
    ...
    _needleView.center = CGPointMake(width/2 - xOffset, height-needleWidth/2);
    [self rotateNeedleByAngle: -90 + _startAngle];
    
    _containerLayer = [CALayer layer];
    _containerLayer.frame = CGRectMake(0 - xOffset, 0, width, height);
...
- (void)setupArcLayers
{
    ...
    _backgroundArcLayer.strokeColor = UIColorFromHex(0xf2f2f2);
    _backgroundArcLayer.fillColor = UIColorFromHex(0xf2f2f2);
...
- (void)fillUpToAngle:(float)angle
{
    if ( _valueArcLayer )
    {
        ...
        // Color gauge area customization
        if (_value == 5) {
            _fillArcFillColor = [UIColor clearColor];
            _fillArcStrokeColor = [UIColor clearColor];
        } else if (_value < 5) {
            _fillArcFillColor = UIColorFromHex(0xd30203);
            _fillArcStrokeColor = UIColorFromHex(0xd30203);
        } else {
            _fillArcFillColor = UIColorFromHex(0x249500);
            _fillArcStrokeColor = UIColorFromHex(0x249500);
        }
        _valueArcLayer.fillColor = _fillArcFillColor;
        _valueArcLayer.strokeColor = _fillArcStrokeColor;
    }
